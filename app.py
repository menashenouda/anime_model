...
INTENTS = {
    "greeting": {
        ...
        "responses": ["Hello! How can I help you today?", "Hi there! Ready for anime recommendations?","Hey, hope you are doing well", "Hey, are you ready to watch new animes today"]
    },
    "leaving": {
    ...
    "services": {
        ...
        "responses": ["I can recommend animes! Just ask for recommendations and mention a anime you like.","I am here to help you finding new animes"]
    },
    "recommendation": {
        "patterns": [r"recommend|suggest|animes like|a anime like"],
    },
    "non_related": {
        "responses": ["Hmm, I'm not sure about that. Let's stick to anime recommendations!","I am not programmed to answer these questions"]
    }
}
...
def correct_spelling(query):
    ...
    for word in words:
        if word.lower() in [t.lower() for t in custom_anime_titles]:
            original_title = next(t for t in custom_anime_titles if t.lower() == word.lower())
...
def extract_anime_titles(query):
...
def chat_endpoint():
    ...
    if intent == "greeting":
        ...
        if not anime_title:
            response = "Could not identify a anime title in your request."
        else:
            try:
                ...
                # Get top 5 similar animes (excluding the input itself)
